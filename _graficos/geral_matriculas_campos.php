...
$sql = "SELECT DATA, MES, TOT_MATRICULAS_CAMPOS FROM graficogeral WHERE DATA >= '2019-08-01'";
$result = mysqli_query($conn, $sql);
while($linha = mysqli_fetch_assoc($result)){
	$resultado[] = $linha;
}

$ult_mat = "SELECT MAT_ULTIMA FROM producao ORDER BY DATA DESC LIMIT 2";
$resultult = mysqli_query($conn, $ult_mat);
while($linha = mysqli_fetch_assoc($resultult)){
  $resultfim[] = $linha;
}

$mat_fim = array();
foreach ($resultfim as $key => $value) {
  $mat_fim[$key] = $resultfim[$key]['MAT_ULTIMA'];
}

$ultima_matricula = max($mat_fim);
...
<?php include "atualiza_geral_matriculas.php"; ?>
...
<script type="text/javascript">
    ...
    */
    ...
    options: {
      ...
      title: {
        display: true,
        text: "PRODUÇÃO GERAL DO SETOR DE RECADASTRO - Total de Matrículas (Campos) : " + "<?php echo $mat_camp; ?>" +   "  -  " + "Última Matrícula: " + "<?php echo $ultima_matricula; ?>",
      }
    }
  });
</script>
